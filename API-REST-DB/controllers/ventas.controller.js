...
const getAllSales = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Hubo un error al obtener las ventas', error: error.message });
    }
};
...
const getSaleById = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Hubo un error al obtener la venta solicitada', error: error.message });
    }
};
...
const createSale = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ status: 201, message: 'Venta registrada exitosamente', data: newSale });
    } catch (error) {
        res.status(500).json({ message: 'Hubo un error al crear la nueva venta', error: error.message });
    }
};
...
const updateSale = async (req, res) => {
    try {
        ...
        await sale.save();
        res.json({ status: 200, message: 'Venta actualizada exitosamente', data: sale });
    } catch (error) {
        res.status(500).json({ message: 'Hubo un error al actualizar la venta', error: error.message });
    }
};
...
const deleteSale = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Hubo un error al eliminar la venta', error: error.message });
    }
};
